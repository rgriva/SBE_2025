...
\begin{frame}{Flight Plan {\normalfont\emoji{airplane}}}
    \begin{enumerate}
        ...
        \item Solving Large-Scale Dynamic Models\tikz[remember picture,baseline] \node (it4) {};
    \end{enumerate}
...
\emoji{flag-brazil} \cite{masini_balancing_2025} \emoji{flag-brazil} proposed some middle ground:
\begin{equation*}
...
\begin{equation*}
    Y = f(X) + \varepsilon
\end{equation*}
A \textbf{Random Tree} (or Regression Tree) is a particular way of parametrizing $f$!
\pause
\vspace{0.5cm}
...
\begin{gather*}
    ...
    \mu_1(\delta) \equiv \sum_{i: x_i \in S_1} \frac{Y_i}{n_1}, \quad \mu_2(\delta) \equiv \sum_{i: x_i \in S_2} \frac{Y_i}{n_2}
\end{gather*}
\pause
\begin{itemize}\itemsep0.4em
    \item Define $SSR(\delta) \equiv \sum_{i: x_i \in S_1} (Y_i - \mu_1(\delta))^2 + \sum_{i: x_i \in S_2} (Y_i - \mu_2(\delta))^2$
...
\begin{gather*}
    ...
    (\hat{\beta}_1) \equiv \arg\min_{\beta} \sum_{i: Z_{i} \in S_1} (Y_i - X_i^\top \beta)^2, \quad (\hat{\beta}_2) \equiv \arg\min_{\beta} \sum_{i: Z_{i} \in S_2} (Y_i - X_i^\top \beta)^2\\
    SSR(\delta) \equiv \sum_{i: Z_{i} \in S_1} (Y_i - X_i^\top \hat{\beta}_1)^2 + \sum_{i: Z_{i} \in S_2} (Y_i - X_i^\top \hat{\beta}_2)^2
\end{gather*}
\pause
...
\begin{frame}{There is tension in the air...}
    \begin{itemize}
        ...
        \pause
        \item In practice: forget solving models with more than $\approx$ 10-15 state variables;
    \end{itemize}
    \vspace{0.6cm}
    \begin{center}
...
\begin{gather*}
    ...
    \pause
    HJB(\s, \cc(\s), V(\s)) = 0, \quad \forall \s \in \R^n \, \text{at the optimum policy}\\
...
\begin{itemize}
    ...
    \begin{equation*}
        \frac{dV(s)}{ds} \Big|_{s=s_i} \approx \frac{V(s_{i+1}) - V(s_{i-1})}{\Delta s}, \quad \frac{d^2V(s)}{ds^2} \Big|_{s=s_i} \approx \frac{V(s_{i+1}) - 2V(s_i) + V(s_{i-1})}{\Delta s^2}
    \end{equation*}
    \pause
...
\begin{itemize}
    ...
    \[
        V(s) = \sum_{k=0}^{K} a_k T_k(s), \quad \text{$\{T_k(.)\}_{k=0}^{K}$ are Chebyshev polynomials}
    \]
    \pause
...
(\emoji{flag-brazil} $\times 3$) \cite{Duarte2024}: let's embrace Neural Networks!
\vspace{0.4cm}
\pause
...
\begin{frame}{Does it have good properties?}
    \cite{Cybenko1989,Hornik1989,Hornik1991} proved that this basis of functions is rich:
...
Recall that we need to find $V(\s)$ and $\cc(\s)$ such that:
\begin{gather*}
    HJB(\s, \cc, V) \equiv -\rho V(\s) + u(\cc) + \nabla_{\s} V(\s)^\top f(\s, \cc) + \frac{1}{2} \text{tr}\left( g(\s, \cc)^\top\nabla^2_{\s\s} V(\s) g(\s, \cc)  \right)\\
    HJB(\s, \cc(s), V(\s)) = 0, \quad \forall \s \in \mathcal{S} \subset \R^n \, \text{at the optimum policy}\\
    \cc(s) = \arg \max_{\cc} \left\{u(\cc) + \nabla_{\s} V(\s)^\top f(\s, \cc) + \frac{1}{2} \text{tr}\left(  g(\s, \cc)^\top \nabla^2_{\s\s} V(\s) g(\s, \cc) \right) \right\}
\end{gather*}
\pause
    \begin{itemize}
        \item Key insight in \cite{Duarte2024}: approximate $V(\s)$ and $\cc(\s)$ using DNNs;
        \item $V(\s) = V(\s, \boldsymbol{\theta}_V)$, $\cc(\s) = \cc(\s, \boldsymbol{\theta}_c)$;
        \item Derivatives of $V(.)$ are super fast to compute using Automatic Differentiation;
        \pause
        \item We look for a pair $(\boldsymbol{\theta}_V, \boldsymbol{\theta}_c)$ that solves minimizes the HJB residuals!
        \item Notation: $HJB(\s, \cc(\s, \boldsymbol{\theta}_c), V(\s, \boldsymbol{\theta}_V)) \equiv HJB(\s, \boldsymbol{\theta}_c, \boldsymbol{\theta}_V)$;
    \end{itemize}
\end{frame}

\begin{frame}{The Algorithm}
    \textbf{Step 1}: sample $I$ points $\{\s_i\}_{i=1}^I$ from the state space $\mathcal{S}$ (for example, uniformly);

    \textbf{Step 2}: Initialize some $(\boldsymbol{\theta}_V^{(0)}, \boldsymbol{\theta}_c^{(0)})$;
    \pause

    \textbf{Step 3}: If we change $\boldsymbol{\theta}_C$ by a little, the HJB error at $\s_i$ moves by $\nabla_{\boldsymbol{\theta}_c} HJB(\s_i, \boldsymbol{\theta}_c, \boldsymbol{\theta}_V)$. 
    At iteration $j$, compute:
    \begin{equation*}
        \boldsymbol{\theta}_c^{(j)} = \boldsymbol{\theta}_c^{(j-1)} + \frac{\gamma}{I} \left( \sum_{i=1}^I \nabla_{\boldsymbol{\theta}_c} HJB\left(\s_i, \boldsymbol{\theta}_c^{(j-1)}, \boldsymbol{\theta}_V^{(j-1)}\right) \right)
    \end{equation*}
    \pause

    \textbf{Step 4}: Define the loss function $\mathcal{L}(\boldsymbol{\theta}_V)$ and compute its gradient:
    \begin{gather*}
        \mathcal{L}(\boldsymbol{\theta}_V) \equiv \frac{1}{2I}\sum_{i=1}^I \left( HJB\left(\s_i, \boldsymbol{\theta}_c^{(j)}, \boldsymbol{\theta}_V\right) \right)^2\\
         \nabla_{\boldsymbol{\theta}_V} \mathcal{L}(\boldsymbol{\theta}_V) = \frac{1}{I} \sum_{i=1}^I HJB\left(\s_i, \boldsymbol{\theta}_c^{(j)}, \boldsymbol{\theta}_V\right) \nabla_{\boldsymbol{\theta}_V} HJB\left(\s_i, \boldsymbol{\theta}_c^{(j)}, \boldsymbol{\theta}_V\right)
    \end{gather*}
\end{frame}

\begin{frame}{The Algorithm (continued)}
    \textbf{Step 5}: Update $\boldsymbol{\theta}_V$ using Gradient Descent:
    \begin{equation*}
        \boldsymbol{\theta}_V^{(j)} = \boldsymbol{\theta}_V^{(j-1)} - \eta \nabla_{\boldsymbol{\theta}_V} \mathcal{L}(\boldsymbol{\theta}_V^{(j-1)}) = \boldsymbol{\theta}_V^{(j-1)} - \frac{\eta}{I} \sum_{i=1}^I HJB\left(\s_i, \boldsymbol{\theta}_c^{(j)}, \boldsymbol{\theta}_V^{(j-1)}\right) \nabla_{\boldsymbol{\theta}_V} HJB\left(\s_i, \boldsymbol{\theta}_c^{(j)}, \boldsymbol{\theta}_V^{(j-1)}\right)
    \end{equation*}
    \pause

    \textbf{Step 6}: Repeat Steps 3-5 until convergence.
    \pause

    \vspace{0.3cm}
    At the end, we have approximations for the value function and policy function:
    \begin{equation*}
        V(\s) \approx V(\s, \boldsymbol{\theta}_V^{(*)}), \quad \cc(\s) \approx \cc(\s, \boldsymbol{\theta}_c^{(*)})
    \end{equation*}
    \pause
    \vspace{-0.8cm}
    \noindent\rule{\linewidth}{0.4pt}
    ...
    Summarizing:
    \begin{itemize}
        \item DNNs are a rich basis of functions with cheap derivatives and not so many parameters;
        \item They are an extremely flexible class;
    \end{itemize}
\end{frame}
...
\begin{frame}{Wrap-Up}
    ...
    \begin{itemize}
        \item Always think about ML methods an array of \textit{functional bases};
        \item A bit of LASSO and how to use it when we have many controls
        \item A bit of Random Forests and how to use them to estimate heterogeneous partial effects;
        \item A tiny drop of Deep Learning and how to tweak it to solve HJB equations;
        \item \href{https://sites.google.com/site/dejanirhsilva/}{Dejanir Silva} has a full class on this: \href{https://github.com/dejanirsilva/mlce}{https://github.com/dejanirsilva/mlce}
    \end{itemize}
\pause
\vspace{1cm}
    \begin{center}
        \fbox{\parbox{0.5\textwidth}{
            \centering
            \alert{\textbf{Important: I hope you had fun!}}
        }}
    \end{center}
\end{frame}
...
\begin{frame}[standout]
    \begin{center}
       {\Huge Thank you!} \emoji{folded-hands-light-skin-tone}

       \vspace{0.7cm}
       Scan below to get in touch!
       \begin{figure}
        \centering
        \includegraphics[width=0.3\textwidth]{../pictures/QR_code.pdf}
       \end{figure}
    \end{center}
\end{frame}
