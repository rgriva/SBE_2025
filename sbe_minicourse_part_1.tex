...
\begin{figure}
    \centering
    \includegraphics[width=0.95\textwidth]{Single_vs_Double_Selection.png}
\end{figure}
\end{frame}
...
\begin{equation*}
    \sqrt{n}(\hat{\theta}_0 - \theta_0) \to_d \mathcal{N}(0, \sigma^2)
\end{equation*}
where $\sigma^2$ is complicated by consistently estimated.
\pause
\vspace{0.5cm}
...
\begin{itemize}
    ...
    \item The secrete sauce: \underline{cross-fitting} $\implies$ efficiency vs strict assumptions;
...
Some open problems:
\begin{itemize}
    \item Weak identification, in special in the IV context;
    \item Time series $\implies$ it's impossible to do cross-fitting;
    \item Panel data $\implies$ usual estimators leverage linearity;
\end{itemize}
\pause
\vspace{1cm}
\begin{center}
